...
像上一节实现的分派有两个显著的弱点。
第一个弱点：
其中的通用型界面过程必须知道所有不同的表示。
第二个弱点：
即使这两种独立的表示形式可以分别设计，我们也必须保证整个系统里不存在两个名字相同的过程。
这种实现通用型界面的技术不具有可加性。

在每次增加一种新表示形式时，实现通用选择函数的人都必须修改他们的过程，而那些做独立表示的界面的人也必须去修改其代码，以避免名字冲突问题。
在做这些事情时，所有修改必须直接对代码修改，而且必须准确无误。
这带来极大的不便，而且容易引进错误。如果假定现在要处理几百种不同表示形式，比如大规模的数据库管理系统。

需要一种能够将系统设计进一步模块化的方法。
一种称为数据导向的程序设计的编程技术提供了这种能力。
理解数据导向的程序设计如何工作？
首先，在需要处理的针对不同类型的一集公共通用型操作时，事实上，我们正是处理一个二维表格，其中的一维上包含着所有操作，另一种维度就是所有的可能类型。
表格中的项目是一些过程，它们针对作为参数的每个类型实现每一个操作。

数据导向的程序设计就是一种使用程序能直接利用这种表格工作的程序设计技术。

实现一个过程，用操作名和参数类型的组合到表格中查找，以便找出应该调用的适当过程，并将这一过程应用于参数的内容。

不需要修改任何现存的过程，而只要在这个表格里添加一些新的项目就可以。
假定有两个过程put和get，用于处理这种操作-类型表格：
(put <op> <type> <item>)
(get <op> <type>)
在表中查找与<op><type>对应的项，如果找到就返回找到的项，否则就返回假。
...
```
定义一个名为apply-generic的操作过程访问有关表格，这个过程将通用型操作应用于一些参数。
该过程在表格中用操作名和参数类型查找，如果找到，就去应用查找中得到的过程。

```
(define (apply-generic op . args)
    (let ((type-tags (map type-tag tags)))
...
2.4.2节中所用的程序设计风格，是一种基于类型进行分派的组织方式，其中让每个操作管理自己的分派。
从效果上看，这种方式就是将操作--类型表格分解为一行一行，每个通用型过程表示表格中的一行。

另一处实现策略是将这一表格按列进行分解，
不是采用一批“智能操作”去基于数据类型进行分派，而是采用“智能数据对象”，让它们基于操作名完成所需的分派工作。
如果我们想这样做，所需要做的是做出一种安排，将每一个数据对象表示为一个过程。
它以操作的名字作为输入，能够去执行指定的操作。
按照这种方式，make-from-real-imag应该写成下面的样子：
...
(define (make-from-real-imag x y)
    (define (dispatch op)
        (cond ((eq? op 'real-part) x)
                 ...
                 (else (error "unknow op -- MAKE-FROME-REAL-IMAG" op))))
    dispatch)
...
```
注意，这里的make-from-real-imag返回的值是一个过程--它内部的dispatch过程。
这是当apply-generic要求执行一个操作时所调用的过程。

这种风格的程序设计称为消息传递，这一名字源自将数据对象设想为一个实体，它以“消息”的方式接收到所需操作的名字。
消息传递并不是一种数学机巧，而是一种有价值的技术，可以用于组织带有通用型操作的系统。

练习 2.75
练习 2.76

## 2.5 带有通用型操作的系统
2.4节讲了如何去设计一个系统，使其中的数据对象可以以多于一种方式表示。
关键思想：
通过通用型界面过程，将描述数据操作的代码连接到几种不同表示上。

针对不同参数种类的通用型操作。

使用数据导向技术，把所有算术包都结合起来。

### 2.5.1 通用型算术运算
设计通用型的加法过程add，对于常规的数，它的行就像常规的基本加法，对于有理数，它就像add-rat，对于复数就像add-complex。
构造函数打标识
把操作名和标识作为参数，去匹配操作表中的过程，取出对应的过程。
最后运算出结果
...
### 2.5.2 不同类型数据的组合
之前都是把所有不同数据类型看作相互完全分离的东西，这里有几个完全分离的程序包，它们分别处理不同类型数据的操作。
但如何去定义一种跨过类型界限的操作，这很有意义，如完成一个复数和常规数的加法。
前面一直是在程序的各个部分之间引进屏障，以使它们可以分别开发和处理。
如何使我们支持跨类型的操作的同时，又没有严重地损害模块间的分界。

处理跨类型操作的一种方式，就是为每一种类型组合的合法运算设计一个特定过程。
可用，但很麻烦。不利于新类型的引入。
在设计包开始要规划好一套统一的策略，分清各种包之间的责任。

强制
最一般的情况是需要处理针对一批完全无关的类型的一批完全无关的操作，直接实现跨类型操作。
也比较麻烦。
可以利用潜藏在类型系统之中的一些额外结构，将事情做得更好些。

不同的数据类型通常都不是完全相互无关的，常常存在一些方式，可以把一种类型的对象看作另一种类型的对象。
这种过程称为强制。
如，当常规数和复数相运算时，可以把常规数看作虚部为0的复数。使用复数运算。

```
(define (scheme-number->complex n)
    (make-complex-from-real-imag (contents n) 0))
```
强制过程安装到一个特殊的强制表格中，用两个类型的名称作为索引
...
修改前面的apply-generic过程，得到一种处理强制的统一方法。
在使用操作时，先检查是否存在针对实际参数类型的操作定义。
如果存在，就将任务分派。否则就去强制。
查看第一个参数类型的对象能否转换为第二个参数类型。
看第二个参数类型是否能转换为第一个参数类型。
...
```
与显示定义的跨类型操作相比，这种强制模式只需要为每一对类型写一个过程，而不是每对类型和每个通用型操作写一个过程。
能够这样做的基础是，类型之间的适当转换只依赖类型本身，而不依赖于所实际应用的操作。


类型的层次结构
上面给出的强制模式，依赖于一对对类型之间存在着某种自然的关系。在实际中，还存在着不同类型间相互关系的更“全局性”的结构。

假定想要构造一个通用型的算术系统，处理整数、有理数、实数、复数。
在这样的系统里，一种很自然的是把整数看成一类特殊的有理数，有理数又是一种特殊的实数，实数转而又是一类特殊的复数。

这时就有了一个类型的层次结构，在其中，整数是有理数的子类型（任何有事数的操作都可以应用于整数）。
有理数形成了整数的一个超类型。

如果我们面对的是一个塔结构，那么将一个新类型加入层次结构的问题就可以极大的简化了，
因为需要做的就是刻画清楚新类型将如何嵌入正好位于它之上的超类型，以及它如何作为下面一个类型的超类型。

例如：
我们希望做一个整数和一个复数的加法，那么不需要明确定义一个特殊的强制函数。相反，可以定义如何将整数转换到有理数，如何将有理数转换到实数，
以及如何将实数转换到复数。而后让系统通过这些步骤将该整数转换到复数，在此之后再做两个复数的加法。

提供一个raise过程，它将这一类型的对象“提升”到塔中更高一层的类型。此后，当系统遇到需要对两个不同类型的运算时，它就可以逐步提升较低的类型，
直至所有对象都达到了塔的同一个层次。

类型塔的另一个优点：很容易实现，每个类型能够“继承”其超类型中定义的所有操作。
举例： 
如果我们没有为找出整数的实部提供一个特定过程，但也完全可能期望real-part过程对整数有定义，因为事实上整数是复数的一个子类型。

塔形结构的另一个优点，它使我们有一种简单的方式去“下降”一个数据对象，使之达到最简单的表示形式。

层次结构的不足
当一个类型有多于一个的子类型，这种存在多重超类型的问题，意味着并不存在一种唯一方式在层次结构中去“提升”一个类型。

当我们需要将一个操作应用于一个对象时，为些找出“正确”超类型的工作可能涉及到对整个类型网络的大范围搜索。

由于一般说一个类型存在着多个子类型，需要在类型层次结构中“下降”一个值时也会遇到类似的问题。
...
### 2.5.3 实例：符号代数
符号表达式的操作，能够展示出在设计大型系统时常常会出现的许多困难问题。
一个表达式可以看做是具有层次结构的东西，它是将运算符作用于一些运算对象而形成的一棵树。
可以从一个基本对象，例如常量和变量出发，通过各种代数运算符如加法和乘法组合，构造起各种各样的代数表达式。
在这里需要形成各种抽象，使我们能够有简单的方式去引用复合对象。
符号代数中，典型的抽象：线性组合，多项式，有理函数和三角函数等。
可以将这些看作是复合的“类型”，它们在制导表达式的处理过程非常有用。

例如：
x^2*sin(y^2+1)+x*cos2y+cos(x^3-2y^2)
看作是一个多项式，其参数是y的多项式的三角函数，而y的多项式的系数是整数。

开发一个完整的代数演算系统。
复杂，包含代数知识和算法。
只处理多项式算术。
下面展示在设计中所面临的各种抉择，以及如何利用抽象数据和通用型操作的思想，以利于组织好项目。

多项式算术
确定多项式到底是什么？
针对某些特写的变量定义的。这里把多项式里的未定义元设置为一个的情况。
将多项式定义为项的和式，每个项就是一个系数，未定元的乘方，一个系数与一个未定元乘方的积。
系数也定义为一个代数表达式，但它不依赖于这个多项式的未定元。
在这里确定一个代数演算系统里，一个“多项式”就是一种特殊的语法形式，而不是在其之下的数学意义。

怎么做多项式算术？
简单系统只处理加法和乘法，强制性地要求两个参与运算的多项式具有相同的未定元。

设计系统
多项式使用称为poly的数据结构表示，它由一个变量和一组项组成。
假定两个选择函数variable和term-list，用于从一个多项式中提取相应的部分。
构造函数make-poly，从给定的变量和项表构造出一个多项式。
一个变量也就是一个符号，使用same-variable?过程做变量的比较。
...
(define (mul-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (make-poly (variable p1) 
                         (mull-terms (term-list p1)
                                          (term-list p2)))
        (error "polys not in same var -- MUL-POLY" (list p1 p2))))

```

为了将多项式结合到前面建立起来的通用算术系统里，我们城要为其提供类型标志。
采用polynomial，并将适合用于带标志多项式的操作安装到操作表格里。
...
(define (install-polynomial-package)
 ...
 'done)
```
多项式加法通过一项项的相加完成，同次的项必须归并到一起。
方法是建立一个同次的新项，其系数是两个项的系数之和。

假定有一个空的项表。构造函数the-empty-termlist。
构造函数adjoin-term将新项加入一个项表里。
以及一个可以检查项表是否为空的函数empty-termlist？
提取一个项表中最高次数的项的函数first-term
返回除最高项外的其他项的函数rest-terms
假定已经有一个构造函数make-term，它从给定的次数和系数构成一个项。
选择函数order和coeff分别返回一个项的次数和系数。
这样可以把项和项表都看成一个数据抽象，其具体实现可以另外再思考。

//todo：这里有许多代码，不打了，之后还要用js重构呢
...
符号代数中类型的层次结构
一个多项式的项系数也是多项式，通用型操作和数据导向的程序设计风格完全可以处理这种复杂性。
练习 2.92

扩充练习：有理函数
有理数函数也就是“分式”，其分子和分母都是多项式。
需要这个系统可以做有理函数的加减乘除。
